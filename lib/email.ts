/**
 * Email service for JohannesJohn.
...
const RESEND_FROM = process.env.RESEND_FROM_EMAIL || process.env.BUSINESS_EMAIL || "JohanessJohn <onboarding@resend.dev>";
const RESEND_REPLY_TO = process.env.RESEND_REPLY_TO_EMAIL || process.env.RESEND_REPLY_TO || process.env.BUSINESS_EMAIL || "sales@johannesjohn.com";

import { getAppUrl } from "./app-url";
const appUrl = getAppUrl();
const withStagingPrefix = (subject: string) =>
  process.env.VERCEL_ENV === "production" || subject.startsWith("[JOHANNESJOHN TEST]")
    ? subject
    : `[JOHANNESJOHN TEST] ${subject}`;
...
export async function sendWelcomeEmail(params: WelcomeEmailParams): Promise<void> {
  const { to, name, storeName = "JohannesJohn" } = params;
...
export async function sendTwoFactorEmail(params: TwoFactorEmailParams): Promise<void> {
  const { to, name, action, resetBy, storeName = "JohannesJohn" } = params;
...
export async function sendRoleChangeEmail(params: RoleChangeEmailParams): Promise<void> {
  const { to, name, changes, changedBy, storeName = "JohannesJohn" } = params;
...
export async function sendInvitationEmail(params: InvitationEmailParams): Promise<void> {
  const { to, name, inviterName, code, token, role, note, storeName = "JohannesJohn" } = params;
...
export async function sendPasswordResetEmail(params: PasswordResetEmailParams): Promise<void> {
  const { to, token, storeName = "JohannesJohn" } = params;
...
export async function sendAccountStatusEmail(params: AccountStatusEmailParams): Promise<void> {
  const { to, name, status, reason, storeName = "JohannesJohn" } = params;
